...
class TopLevel:
    ...
    def create_syn_file(self, out_file : str, entity_collection : EntityCollection, 
        ...
        # Get rendering template. Loader rooted at the script directory so the template path
        # stays relative; works cross-platform (the previous FileSystemLoader("/") variant
        # failed on Windows because Jinja2 splits template names by '/').
        env = Environment(loader=FileSystemLoader(os.path.dirname(__file__)))
        template = env.get_template("top.template")
        ...
        data = {
            ...
            "timing_regs" : os.environ.get("OLO_TIMING_REGS") == "1"
        }
