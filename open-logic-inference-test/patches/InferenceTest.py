            ...
            for tool_name, tool in tools.items():
                    ...
                    for config in configs:
                        ...
                        if not args.dry_run:
                            ...
                            #Check DRCs
                            tool.check_drc()
                            #Fmax (Libero timing flow, enabled via OLO_TIMING_REGS=1)
                            if os.environ.get("OLO_TIMING_REGS") == "1" and hasattr(tool, "get_fmax"):
                                fmax_mhz = tool.get_fmax()
                                f.write(f"# {entity_name} {config}: Fmax(Clk) = {fmax_mhz:.3f} MHz\n")
                                f.flush()
                                rpt_src = os.path.join(tool.PROJECT_FOLDER, "max_timing.rpt")
                                if os.path.exists(rpt_src):
                                    shutil.copy(rpt_src, os.path.join(OUT_PATH, f"{entity_name}__{config}__max_timing.rpt"))
                        end = datetime.now()
                        runtime = end - start
                        runtime_str = f"{runtime.seconds // 60:02}:{runtime.seconds % 60:02}"
                        if os.environ.get("OLO_TIMING_REGS") == "1":
                            print(f"[{runtime_str}]  Fmax(Clk)={fmax_mhz:.1f} MHz")
                        else:
                            print(f"[{runtime_str}]")
